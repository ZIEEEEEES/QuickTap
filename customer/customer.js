    ...
    .then(({ data: promos, error }) => {
      ...
      activePromos.forEach((d) => {
        ...
        if (d.image_url) {
          const img = document.createElement("img")
          img.src = d.image_url
          img.alt = d.title || "Promo Image"
          img.className = "promo-card-image"
          img.style.width = "100%"
          img.style.height = "auto"
          img.style.maxHeight = "500px" // Limit height but show full width
        ...
        if (d.created_at) {
            date.textContent = "Posted: " + new Date(d.created_at).toLocaleDateString()
        }

        const expiry = document.createElement("div")
        expiry.className = "promo-expiry"
        if (d.valid_until) {
            expiry.innerHTML = `Valid until: <span style="color: var(--preorder-accent); font-weight: 600;">${new Date(d.valid_until).toLocaleDateString()}</span>`
        }
...
async function runTrackOrder() {
  ...
  try {
      ...
      historyOrdersData.forEach(item => {
          ...
          if (!historyGroups[key]) {
              historyGroups[key] = {
                  ...
                  rejection_reason: item.rejection_reason || item.rejectionReason || item.insufficient_notes || item.notes || item.rejection_reason // Support items loop
              }
          }
          ...
          if (order.status === 'completed' || order.status === 'rejected' || order.status === 'cancelled') {
              ...
              if (!historyGroups[key]) {
                  ...
                  historyGroups[key] = {
                      ...
                      total: order.total,
                      status: order.status,
                      rejection_reason: order.rejection_reason || order.rejectionReason || order.insufficient_notes || order.notes || (Array.isArray(items) ? items.find(i => i.rejection_reason)?.rejection_reason : "")
                  }
              }
          }
      })
      ...
      const renderOrderRow = (d, isHistory = false) => {
            ...
            let reasonDisplay = ""
            if (s === 'rejected') {
                // Point: Exhaustive search for the reason (columns + items JSON)
                let itemReason = ""
                if (Array.isArray(items) && items.length > 0) {
                    itemReason = items.find(i => i.rejection_reason)?.rejection_reason || ""
                }
                reasonDisplay = d.rejection_reason || d.rejectionReason || d.insufficient_notes || d.notes || itemReason || "No reason provided"
                ...
                reasonDisplay = `\u26A0 Insufficient: Need \u20B1${stillNeeded.toFixed(2)}`
            }
