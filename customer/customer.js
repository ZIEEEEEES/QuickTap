...
// Cafe hours modal functions
window.openCafeHoursModal = () => {
  const modal = document.getElementById('cafeHoursModal');
  if (modal) {
    modal.style.display = 'flex';
  }
}

window.closeCafeHoursModal = () => {
  const modal = document.getElementById('cafeHoursModal');
  if (modal) {
    modal.style.display = 'none';
  }
...
function subscribeToTrackOrderRealtime() {
    ...
    const loyaltyCard = currentCustomer.loyalty_card
    
    const isMatch = (custId, loyaltyCardField) => {
        const orderCustId = String(custId || "").toLowerCase()
        const orderLoyaltyCard = String(loyaltyCardField || "").toLowerCase()
        ...
        const matchesLoyalty = loyaltyCard && (
            orderCustId.includes(loyaltyCard.toLowerCase()) || 
            (orderLoyaltyCard && orderLoyaltyCard.includes(loyaltyCard.toLowerCase()))
        )
        return matchesEmail || matchesContact || matchesLoyalty
    }
    ...
    trackOrderUnsub = db.channel('customer-tracking-realtime')
        .on('postgres_changes', { 
            ...
            if (order && isMatch(order.customer_id, order.loyalty_card)) {
        ...
        .on('postgres_changes', { 
            ...
            if (booking && isMatch(booking.customer_id, booking.loyalty_card)) {
...
window.submitVisitFromModal = () => {
  ...
  // Validate times are within cafe hours
  const parseHM = (v) => {
    const [h, m] = v.split(":").map(Number)
    return { total: h * 60 + m }
  }
  const visitStart = 8 * 60
  const visitEnd = 19 * 60
  ...
  if (start >= end) {
    if (msg) msg.innerHTML = `<div class="message error">Check-out time must be after check-in time.</div>`
    return
  }
  const typeEl = document.getElementById("bookType")
...
async function fetchInsufficientOrdersForNotifications() {
  ...
  try {
    ...
    pendingList.forEach((o) => {
      ...
      // Don't show if status is ready/for_pickup
      const statusLower = String(o.status || "").toLowerCase()
      if (['ready', 'for_pickup', 'for pickup', 'completed'].includes(statusLower)) return
      
      const notesText = String(o.insufficient_notes || o.notes || "")
      let remaining = Number(o.insufficient_amount_needed || 0)
      if (remaining <= 0) {
        remaining = parseRemainingFromNotes(notesText)
      }
...
async function fetchPickupNotifications() {
  if (!currentCustomer || !db || isGuest) return []
  const results = []
  const currentReadyPendingIds = new Set()
  const currentReadyBookingIds = new Set()
  ...
  try {
    ...
    pendingList.forEach((o) => {
      ...
      currentReadyPendingIds.add(key)
      results.push({
  ...
  try {
    ...
    bookingList.forEach((o) => {
      ...
      currentReadyBookingIds.add(key)
      results.push({
  ...
  // Remove keys from seenPickupNotifications that are no longer ready (completed/cancelled)
  const keysToRemove = []
  seenPickupNotifications.forEach((key) => {
    if (!currentReadyPendingIds.has(key) && !currentReadyBookingIds.has(key)) {
      keysToRemove.push(key)
    }
  })
  keysToRemove.forEach((key) => {
    seenPickupNotifications.delete(key)
  })
...
function renderCustomerNotifications(list) {
  ...
  data.forEach((n) => {
    ...
    const isCompleted = n.type === "completed" || message.includes("order is complete") || message.includes("order complete")
    if (isPickup) {
      item.innerHTML = `
      ...
      item.innerHTML = `
        <div class="notification-title" style="color: #1565c0;">Order Complete</div>
        <div class="notification-meta">Order #${n.order_id || "--"}</div>
      `
      item.style.borderLeft = "5px solid #1976d2"
      ...
      // Skip notifications with remaining <= 0
      return
    }
...
async function fetchCustomerNotifications({ autoOpen = false } = {}) {
  ...
  try {
    ...
    if (pickup.length) {
      // Replace any existing notification with pickup notification for same order
      const existingKeysMap = new Map(customerNotifications.map((n) => [`${n.source_table}:${n.order_id}`, n]))
      pickup.forEach((f) => {
        const key = `${f.source_table}:${f.order_id}`
        // Remove any existing notification for same order
...
async function registerCustomer() {
  ...
  if (!phone || !name) {
    showMessage("Please enter phone number and name", "error")
    return
  }
...
async function findUserByContact(rawEmail, phone) {
  const email = rawEmail ? rawEmail.toLowerCase() : null
  console.log("[findUserByContact] Inputs:", { rawEmail, email, phone })
  try {
    // Fetch all customers first, then filter manually (case-insensitive)
    const { data: allCustomers, error: fetchError } = await db.from("customers").select("*").order('created_at', { ascending: false })
    if (fetchError) {
      console.warn("Error finding user:", fetchError)
      return null
    }
    
    console.log("[findUserByContact] All customers from DB:", allCustomers)
    if (!allCustomers || allCustomers.length === 0) return null
    
    // Find matching customer
    const user = allCustomers.find(c => {
      const cEmail = (c.email || '').toLowerCase()
      const cContact = (c.contact || '').toLowerCase()
      const cPhone = (c.phone || '')
      
      console.log("[findUserByContact] Checking customer:", {
      ...
      if (email && (cEmail === email || cContact === email)) return true
      if (phone && (cPhone === phone || cContact === phone)) return true
      return false
    })
    
    console.log("[findUserByContact] Found user:", user)
    return user || null
  } catch (err) {
...
async function registerFromModal() {
  ...
  const rawEmail = document.getElementById("modalEmail").value.trim()
  const email = rawEmail ? rawEmail.toLowerCase() : null
  ...
  if (!phone || !name || !password) {
    showMessage("Please enter phone number, name, and password", "error", "modalMessage")
    return
  }
...
async function loginFromModal() {
  ...
  if ((!email && !phone) || !password) {
    showMessage("Enter phone number (or email) and password", "error", "modalMessage")
    return
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const checkDb = () => {
    if (isDbReady()) {
      ...
      const email = rawEmail ? rawEmail.toLowerCase() : null
      const contact = rawContact ? rawContact.toLowerCase() : (email || phone)
      ...
      if (showAuthParam) {
        ...
        findUserByContact(email, phone).then(existingUser => {
            if (existingUser) {
                ...
                window.location.href = '../index.html?error=' + encodeURIComponent("Customer is already registered.");
                return;
            }
            ...
            db.from("customers")
              ...
              .then(({ error }) => {
                ...
                currentCustomer = { email: email || null, phone: phone || null, name, contact: contact, loyalty_card: loyaltyCard, loyalty_points: 0 }
                isGuest = false
                startPortal()
              })
              .catch((error) => {
    ...
    catsToRender.forEach(catName => {
        ...
        filteredProductNames.forEach(prodName => {
            ...
            sizes.forEach(product => {
                ...
                const productImage = product.image_url || product.photo || product.image || ""
                ...
                if (currentQty > 0) {
                    actionArea.innerHTML = `
                        <div class="qty-control-group preorder-qty-group" style="display: flex; justify-content: center; align-items: center; gap: 4px;">
                            <button type="button" class="qty-btn" style="width:20px; height:20px; padding:0; font-size:11px;" onclick="event.stopPropagation(); updatePreorderQty('${product.id}', '${safeName}', '${safeSize}', ${price}, -1, ${selectedTemp ? `'${selectedTemp}'` : 'null'}, '${safeImage}')">-</button>
                    ...
                    addBtn.onclick = (e) => {
                        e.stopPropagation()
                        updatePreorderQty(product.id, safeName, safeSize, price, 1, selectedTemp, productImage)
                    }
...
function updatePreorderQty(id, name, size, price, change, temperature = null, image = "") {
...
window.openPaymentFromPreorder = function() {
    showPreorderPaymentView()
}
...
function renderPreorderCart() {
    ...
    items.forEach(item => {
        ...
        // Try to find image
        const photo = item.image_url || item.photo || item.image || ""
        const imgHtml = photo 
...
window.validateReceiptImage = (fileInput, type = 'preorder') => {
    ...
    const previewContainerId = type === 'payment' ? 'paymentProofPreviewContainer' : (type === 'repay' ? 'repayProofPreviewContainer' : null)
    const previewImgId = type === 'payment' ? 'paymentProofPreview' : (type === 'repay' ? 'repayProofPreview' : null)
    if (!warningEl) return
    ...
    // Check image dimensions for clarity and show preview
...
window.removeRepayProof = () => {
  const fileInput = document.getElementById('repayProof')
  const previewContainer = document.getElementById('repayProofPreviewContainer')
...
function openPaymentModal() {
    ...
    try {
        // Clear previous receipt preview and file input
        if (typeof removePaymentProof === "function") {
            removePaymentProof();
        }
...
window.closePaymentModal = () => {
    // Clear receipt preview when closing
    if (typeof removePaymentProof === "function") {
        removePaymentProof();
    }
    const modal = document.getElementById("paymentModal")
...
window.openRepayModal = (source, orderId, remainingAmount, notificationId) => {
    // Clear previous repay receipt preview and file input
    if (typeof removeRepayProof === "function") {
        removeRepayProof();
    }
    repayContext = {
...
window.closeRepayModal = () => {
    // Clear repay receipt preview when closing
    if (typeof removeRepayProof === "function") {
        removeRepayProof();
    }
    const modal = document.getElementById("repayModal")
...
async function placeKioskOrder(paymentDetails = {}) {
  ...
  try {
      ...
      // Insert customer notification if online payment
      if (paymentDetails.payment_method === 'online' && insertedOrder && insertedOrder.length > 0) {
        const orderId = insertedOrder[0].id;
        const cleanCustomerId = payload.customer_id;
        const nowIso = new Date().toISOString();
        ...
        await db.from("customer_notifications").insert({
          ...
          remaining_amount: 0,
          status: "unread",
          message: "Your online payment has been submitted. Please wait for verification.",
          created_at: nowIso,
          updated_at: nowIso
        });
      }
...
async function runTrackOrder() {
  ...
  const buildIdentityFilter = (query, { includeLoyaltyCard = true } = {}) => {
    // Escape single quotes for filter string if any
    const e = String(email || "").replace(/'/g, "''")
    const c = String(contact || "").replace(/'/g, "''")
    const loyaltyCard = String(currentCustomer.loyalty_card || "").replace(/'/g, "''")

    const conditions = []
    if (e) {
      conditions.push(`customer_id.eq."${e}"`)
      conditions.push(`customer_id.ilike."%${e}%"`)
      if (includeLoyaltyCard) conditions.push(`loyalty_card.eq."${e}"`)
    }
    if (c) {
      conditions.push(`customer_id.eq."${c}"`)
      conditions.push(`customer_id.ilike."%${c}%"`)
      if (includeLoyaltyCard) conditions.push(`loyalty_card.eq."${c}"`)
    }
    if (loyaltyCard) {
      conditions.push(`customer_id.eq."${loyaltyCard}"`)
      conditions.push(`customer_id.ilike."%${loyaltyCard}%"`)
      if (includeLoyaltyCard) conditions.push(`loyalty_card.eq."${loyaltyCard}"`)
    }
  ...
  try {
      ...
      // 1. Fetch ALL pending orders first, then filter locally (for debugging)
      const { data: allPData, error: allPError } = await db.from("pending_orders").select("*").neq("type", "redemption").limit(50).order('created_at', { ascending: false })
      if (allPError) throw allPError
      console.log("[DEBUG] ALL recent pending orders (unfiltered):", allPData)
      
      // Now filter them locally using the same logic as isMatch()
      const filterLocalOrders = (orders) => {
        if (!currentCustomer) return []
        const email = String(currentCustomer.email || "").toLowerCase()
        const contact = String(currentCustomer.contact || "").toLowerCase()
        const loyaltyCard = String(currentCustomer.loyalty_card || "").toLowerCase()
        
        return orders.filter(o => {
          const orderCustId = String(o.customer_id || "").toLowerCase()
          const orderLoyaltyCard = String(o.loyalty_card || "").toLowerCase()
          
          const matchesEmail = email && (orderCustId.includes(email) || orderLoyaltyCard.includes(email))
          const matchesContact = contact && (orderCustId.includes(contact) || orderLoyaltyCard.includes(contact))
          const matchesLoyalty = loyaltyCard && (orderCustId.includes(loyaltyCard) || orderLoyaltyCard.includes(loyaltyCard))
          
          console.log("[DEBUG] Checking order:", o.id, "- matchesEmail:", matchesEmail, "matchesContact:", matchesContact, "matchesLoyalty:", matchesLoyalty)
          return matchesEmail || matchesContact || matchesLoyalty
        })
      }
      
      allPendingOrders = filterLocalOrders(allPData || [])
      console.log("[DEBUG] allPendingOrders (LOCAL FILTERED):", allPendingOrders)

      // 2. Fetch history orders with customer filter (no loyalty_card column)
      let historyQuery = db.from("orders").select("*")
      historyQuery = buildIdentityFilter(historyQuery, { includeLoyaltyCard: false })
      ...
      // 3. Fetch bookings with customer filter (no loyalty_card column)
      let bookingsQuery = db.from("bookings").select("*")
      bookingsQuery = buildIdentityFilter(bookingsQuery, { includeLoyaltyCard: false })
      ...
      allOrders = [
        ...
        ...allBookings.filter(b => 
            b.type === 'preorder' && 
            ['pending', 'accepted', 'preparing', 'ready', 'rescheduled'].includes(b.status)
        )
      ]
      ...
      let activeBookingsSection = allBookings.filter(b => {
        if (['pending', 'accepted', 'preparing', 'ready', 'rescheduled'].includes(b.status)) {
          return true
        }
        if (['rejected', 'cancelled'].includes(b.status)) {
          const bookingDate = new Date(b.created_at || b.timestamp || 0)
          const now = new Date()
          const daysDiff = (now - bookingDate) / (1000 * 60 * 60 * 24)
          return daysDiff < 3 // Keep rejected/cancelled bookings in active for <3 days
        }
        return false
      })
      // Sort active bookings so latest is on top
      activeBookingsSection.sort((a, b) => new Date(b.created_at || b.timestamp || 0) - new Date(a.created_at || a.timestamp || 0))
      ...
      const bookingHistory = allBookings.filter(b => {
        ...
        if (['rejected', 'cancelled'].includes(b.status)) {
          ...
          const now = new Date()
          const daysDiff = (now - bookingDate) / (1000 * 60 * 60 * 24)
          return daysDiff >= 3 // Move rejected/cancelled bookings to history after 3 days
        }
        return false
      })
      ...
      const mapOrderStatus = (status, currentlyPreparing, isPaid, hasInsufficientMarker, paidMarker, isPreorder = false, pickupDate = null, isBooking = false) => {
        ...
        if (isPreorder) {
          ...
          if (['preparing', 'to_prepare', 'accepted'].includes(s) && pickupDate && isSameDay(pickupDate, today)) return "To Prepare"
      ...
      const getBadgeStyle = (displayStatus) => {
        ...
        if (displayStatus === "Accepted") return "background:#2196F3;color:white;font-weight:700;"
        if (displayStatus === "Rescheduled") return "background:#FF9800;color:white;font-weight:700;"
        if (displayStatus === "Pending") return "background:#FFC107;color:#333;font-weight:700;"
      ...
      const renderOrderRow = (d, section = "processing") => {
            ...
            const pickupDate = isPreorder ? d.pickup_date || d.date : null
            ...
            let displayStatus = mapOrderStatus(d.status, d.currently_preparing, isPaid, insuff.hasMarker, paidMarker, isPreorder, pickupDate)
            const badgeStyle = getBadgeStyle(displayStatus)

            let itemsHtml = items.map(i => `<div style="display: flex; gap: 8px; align-items: center; margin-bottom: 4px; white-space: normal;">
              <span style="font-weight: 700; min-width: 30px; white-space: nowrap;">${i.quantity || i.qty || 1}x</span>
              <span style="font-weight: 600; white-space: normal;">${i.name || i.product || "Item"}</span>
            </div>`).join("")

            if (isPreorder) {
              itemsHtml += `<div style="font-weight:800;color:#d4a574;text-transform:uppercase;font-size:9px;letter-spacing:1px;margin-top:8px;">Pre-order Pickup</div>`
            }
            ...
            tr.innerHTML = `
              <td colspan="${section === 'toprepare' || section === 'topickup' ? '4' : '6'}" style="padding: 0;">
                <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: flex-start; padding: 16px; border-radius: 12px; background: white; box-shadow: 0 2px 8px rgba(0,0,0,0.05); white-space: normal; word-break: normal; overflow-wrap: normal; min-width: 0; width: 100%;">
                  <div style="flex: 1 1 auto; min-width: 200px; max-width: calc(100% - 120px);">
                    <div style="display: flex; flex-direction: row; gap: 8px; margin-bottom: 8px; align-items: center; white-space: normal; flex-wrap: wrap;">
                      <span style="font-weight: 700; font-size: 0.9rem; color: var(--coffee-dark); white-space: nowrap;">${typeLabel}</span>
                      ${section === "processing" ? `<span class="badge" style="${badgeStyle}; white-space: nowrap;">${displayStatus}</span>` : ''}
                    </div>
                    <div style="margin-bottom: 8px; white-space: normal;">${itemsHtml}</div>
                    ${section !== "toprepare" && section !== "topickup" ? `<div style="font-weight: 800; color: var(--coffee-dark); font-size: 1.1rem; white-space: nowrap;">\u20B1${Number(orderTotal).toFixed(2)}</div>` : ''}
                  </div>
                  <div style="flex: 0 0 auto; width: 110px; text-align: right; display: flex; flex-direction: column; gap: 8px; justify-content: center; white-space: normal;">
                    <div style="color: #888; font-size: 0.8rem; white-space: normal; line-height: 1.3;">${ts}</div>
                  </div>
                </div>
              </td>
            `
      ...
      const renderBookingRow = (d) => {
            ...
            const isPreorder = d.type === "preorder"
            const pickupDate = isPreorder ? d.pickup_date || d.date : null
            let displayStatus = mapOrderStatus(d.status, d.currently_preparing, d.paymentStatus === 'paid', insuffStatus.hasMarker, false, isPreorder, pickupDate, true)
            const badgeStyle = getBadgeStyle(displayStatus)
            let typeLabel = d.type === "preorder" ? "Pre-order" : "Visit"

            let additionalDetails = ""
            if (displayStatus === "Rejected") {
              const reason = d.rejection_reason || d.rejectionReason || d.notes || d.insufficient_notes || ""
              if (reason) {
                additionalDetails = `<div style="margin-top: 8px; color: #d32f2f; font-size: 0.85rem;"><strong>Reason:</strong> ${reason}</div>`
              }
            } else if (displayStatus === "Rescheduled") {
              const reason = d.reschedule_reason || d.notes || d.insufficient_notes || ""
              const newDate = d.new_date || ""
              const newTime = d.new_time || ""
              if (reason || newDate) {
                additionalDetails = `<div style="margin-top: 8px;">`
                if (reason) {
                  additionalDetails += `<div style="color: #e65100; font-size: 0.85rem;"><strong>Reason:</strong> ${reason}</div>`
                }
                if (newDate) {
                  additionalDetails += `<div style="color: #2e7d32; font-size: 0.85rem;"><strong>New Date:</strong> ${newDate}</div>`
                }
                if (newTime) {
                  additionalDetails += `<div style="color: #2e7d32; font-size: 0.85rem;"><strong>New Time:</strong> ${newTime}</div>`
                }
              ...
              <td colspan="5" style="padding: 0;">
                <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: flex-start; padding: 16px; border-radius: 12px; background: white; box-shadow: 0 2px 8px rgba(0,0,0,0.05); white-space: normal; word-break: normal; overflow-wrap: normal; min-width: 0; width: 100%;">
                  <div style="flex: 1 1 auto; min-width: 200px; max-width: calc(100% - 120px);">
                    <div style="display: flex; flex-direction: row; gap: 8px; margin-bottom: 8px; align-items: center; white-space: normal; flex-wrap: wrap;">
                      <span style="font-weight: 700; font-size: 0.9rem; color: var(--coffee-dark); white-space: nowrap;">${typeLabel}</span>
                      <span class="badge" style="${badgeStyle}; white-space: nowrap;">${displayStatus}</span>
                    </div>
                    <div style="margin-bottom: 4px; font-weight: 600; white-space: nowrap;">${d.date || ""}</div>
                    <div style="color: #666; white-space: nowrap;">${time}</div>
                    ${additionalDetails}
                  </div>
                  <div style="flex: 0 0 auto; width: 110px; text-align: right; display: flex; flex-direction: column; gap: 8px; justify-content: center; white-space: normal;">
                    <div style="color: #888; font-size: 0.8rem; white-space: normal; line-height: 1.3;">${ts}</div>
      ...
      const getOrderDisplayStatus = (o) => {
        const isPreorder = o.type === 'preorder'
        const pickupDate = isPreorder ? o.pickup_date || o.date : null
        const isPaid = o.paymentStatus === 'paid' || o.status === 'paid' || o.status === 'PAID'
        const hasInsufficient = resolveInsufficientInfo(o).hasMarker
        return mapOrderStatus(o.status, o.currently_preparing, isPaid, hasInsufficient, false, isPreorder, pickupDate)
      }

      const processingSection = allOrders.filter(o => String(getOrderDisplayStatus(o)).toLowerCase() === 'processing')
      console.log("[DEBUG] processingSection:", processingSection)
      const bookingsSection = activeBookingsSection
      const toPrepareSection = allOrders.filter(o => String(getOrderDisplayStatus(o)).toLowerCase() === 'to prepare')
      console.log("[DEBUG] toPrepareSection:", toPrepareSection)
      const toPickupSection = allOrders.filter(o => String(getOrderDisplayStatus(o)).toLowerCase() === 'to pick up')
      console.log("[DEBUG] toPickupSection:", toPickupSection)
      const completedSectionOrders = allOrders.filter(o => String(getOrderDisplayStatus(o)).toLowerCase() === 'completed')
