...
function createDescriptiveChart(labels, values, type, titleText) {
  ...
  descChartInstance = new window.Chart(freshCanvas, {
    ...
    options: {
      ...
      animation: false,
      plugins: {
        title: { display: true, text: titleText || (isQty ? "Quantity Trend" : "Sales Trend") },
        legend: { position: "top" },
        tooltip: {
          callbacks: { label: (ctx) => ctx.dataset.label + ": " + valuePrefix + Number(ctx.parsed.y || ctx.parsed).toFixed(isQty ? 0 : 2) + valueSuffix },
        },
      },
      scales: { y: { beginAtZero: true, ticks: { callback: (v) => valuePrefix + v + valueSuffix } } },
    },
  })
}
...
function createForecastChart(pastLabels, pastValues, interval) {
  ...
  const mode = document.getElementById("catMode")?.value || "amount"
  const isQty = mode === "quantity"
  const labelPrefix = isQty ? "Actual Items" : "Actual (₱)"
  const valuePrefix = isQty ? "" : "₱"
  const valueSuffix = isQty ? " Qty" : ""
  ...
  predChartInstance = new window.Chart(freshCanvas, {
    ...
    options: {
      ...
      animation: false,
      plugins: {
        title: { display: true, text: isQty ? "Quantity Forecast" : "Sales Forecast" },
        legend: { position: "top" },
        tooltip: { callbacks: { label: (ctx) => valuePrefix + Number(ctx.parsed.y || ctx.parsed).toFixed(isQty ? 0 : 2) + valueSuffix } },
      },
      scales: { y: { beginAtZero: true, ticks: { callback: (v) => valuePrefix + v + valueSuffix } } },
    },
  })
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              const mode = document.getElementById("catMode")?.value || "amount"
              const isQty = mode === "quantity"
              const value = isQty ? Math.round(ctx.parsed) : Number(ctx.parsed).toFixed(2)
              const prefix = isQty ? "" : "₱"
              const suffix = isQty ? " items" : ""
              const pct = Math.round((ctx.parsed / total) * 100)
              return ctx.label + ": " + prefix + value + suffix + " (" + pct + "%)"
            },
          },
        },
      },
...
async function runDashboard() {
  ...
  try {
    ...
    const mode = document.getElementById("catMode")?.value || "amount"
    const isQty = mode === "quantity"
    const suffix = isQty ? " (Quantity)" : " Sales"

    let descTitle = "Daily" + suffix
    if (interval === "week") descTitle = "Weekly" + suffix
    else if (interval === "month") descTitle = "Monthly" + suffix
    else if (interval === "quarter") descTitle = "Quarterly" + suffix
    else if (interval === "semiannual") descTitle = "Semi-Annual" + suffix
    else if (interval === "annual") descTitle = "Annual" + suffix
    ...
    const trendHeader = document.getElementById("trendTitle")
    if (trendHeader) trendHeader.innerText = isQty ? "Quantity Trend" : "Sales Trend"
    
    const catHeader = document.getElementById("categoryTitle")
    if (catHeader) catHeader.innerText = isQty ? "Quantity by Category" : "Sales by Category"
    ...
    if (avgEl) {
        if (isQty) {
            avgEl.innerText = avg.toFixed(1) + " items"
        } else {
    ...
    let catTitlePrefix = isQty ? "Quantity by Category" : "Sales by Category"
    if (interval === "day") catTitlePrefix = (isQty ? "Daily Qty" : "Daily Sales")
    else if (interval === "week") catTitlePrefix = (isQty ? "Weekly Qty" : "Weekly Sales")
    else if (interval === "month") catTitlePrefix = (isQty ? "Monthly Qty" : "Monthly Sales")
    else if (interval === "quarter") catTitlePrefix = (isQty ? "Quarterly Qty" : "Quarterly Sales")
    else if (interval === "semiannual") catTitlePrefix = (isQty ? "Semi-Annual Qty" : "Semi-Annual Sales")
    else if (interval === "annual") catTitlePrefix = (isQty ? "Annual Qty" : "Annual Sales")
...
window.downloadSalesCSV = async function() {
    ...
    try {
        ...
        if (start && end) {
            ...
            if (sY === eY && sM === eM) {
                rangeStr = (sD === eD) ? `${sY}_${sM}_${sD}` : `${sY}_${sM}_${sD}-${eD}`;
            } else if (sY === eY) {
                rangeStr = `${sY}_${sM}_${sD}-${eM}_${eD}`;
            } else {
                rangeStr = `${sY}_${sM}_${sD}-${eY}_${eM}_${eD}`;
            }
        } else {
            const now = new Date();
        ...
        a.download = `Overall Sales ${rangeStr}.csv`; 
...
window.downloadReportsCSV = async function() {
    ...
    try {
        ...
        if (start && end) {
            ...
            // Fallback to today if no range is selected
            const now = new Date();
            const y = now.getFullYear();
            const m = String(now.getMonth() + 1).padStart(2, '0');
            const d = String(now.getDate()).padStart(2, '0');
        ...
        a.download = `Sales Report ${rangeStr}.csv`; 
        document.body.appendChild(a);
        a.click(); 
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
...
window.downloadProductSalesCSV = async function() {
  ...
  try {
    ...
    if (queryStart && queryEnd) {
        ...
        rangeStr = `${queryStart}_to_${queryEnd}`;
    }

    a.download = `Product Performance ${rangeStr}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
