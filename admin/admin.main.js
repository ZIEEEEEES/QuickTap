...
window.updatePromoPreview = () => {
  const title = document.getElementById("promoTitle").value.trim() || "Promo Title"
  const content = document.getElementById("promoContent").value.trim() || "Promo details will appear here..."
  const validUntil = document.getElementById("promoUntil").value
  
  const titleEl = document.getElementById("previewPromoTitle")
  const contentEl = document.getElementById("previewPromoContent")
  const expiryEl = document.getElementById("previewPromoExpiry")
  ...
  if (expiryEl) {
    if (validUntil) {
      expiryEl.innerHTML = `Valid until: <span style="color: var(--accent-warm); font-weight: 700;">${new Date(validUntil).toLocaleDateString()}</span>`
    } else {
...
window.loadPromos = async function() {
  ...
  try {
    ...
    if (!promos || promos.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5">No promos found.</td></tr>'
      return
    }
    ...
    activePromos.forEach((p) => {
      ...
      const photoHTML = p.image_url 
        ? `<img src="${p.image_url}" class="promo-thumbnail" onerror="this.style.display='none'">`
        : '<div class="promo-thumbnail" style="display:flex;align-items:center;justify-content:center;background:#f5f5f5;color:#aaa;font-size:10px;">No Photo</div>'
...
function resetPromoForm() {
    ...
    // Set min date to today for both from and until
    const now = new Date()
    const todayStr = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0')
    const fromInput = document.getElementById("promoFrom")
    const untilInput = document.getElementById("promoUntil")
...
window.addPromoSubmit = async function addPromoSubmit() {
  ...
  try {
    ...
    if (error && (error.message.includes("valid_from") || error.code === "PGRST204" || error.message.includes("Could not find column"))) {
        console.warn("[Admin] Column error, retrying without extended fields");
    ...
    showMessage("Error adding promo: " + (err.message || "Unknown error"), "error")
  } finally {
    ...
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (error && (error.message.includes("valid_from") || error.code === "PGRST204" || error.message.includes("Could not find column"))) {
            ...
            const fallbackUpdate = { title, content };
